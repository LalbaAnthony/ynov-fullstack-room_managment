...
const onProxyReq = (
  ...
  console.log(
    `[PROXY] -> Forwarding request: ${req.method} ${req.url} to ${proxyReq.protocol}//${proxyReq.host}${proxyReq.path}`,
  );
};
...
const onProxyRes = (
  ...
  console.log(
    `[PROXY] <- Received response from target: ${proxyRes.statusCode} ${proxyRes.statusMessage}`,
  );
};
